...
/** Follows the example at https://pytorch.org/tutorials/beginner/pytorch_with_examples.html
  * Same as [[PytorchOrgPolynomialNetworkOptimTutorial]], but uses manual updates instead of a
  * [[com.microsoft.scalatorch.torch.optim.Optimizer]].
  */
object PytorchOrgPolynomialNetworkTutorial {
  def main(args: Array[String]): Unit = ReferenceManager.forBlock { implicit rm =>
    ...
    for (t <- 0 until 2000) {
      ...
      torch.no_grad {
        parameters.foreach(p => if (p.grad.defined()) p.grad.zero_())
      }
